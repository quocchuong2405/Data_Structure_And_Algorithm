...
public:
    ...
    public:
        Iterator(Queue<T>* quEue=0, bool begin=true){
            this->quEue = quEue;
            if(begin){
                if(quEue != 0) this->dllIt = quEue->DLL.begin();
                else this->dllIt = 0;
            }
            else{
                if(quEue != 0) this->dllIt = quEue->DLL.end();
                else this->dllIt = 0;
            }
        }
        Iterator& operator=(const Iterator& iterator ){
...
public:
    ...
    bool add(Node*& _pRoot, Node* new_node) { //
        ...
        while (temp != nullptr) {
            prev = temp;
            num_compare++;
            if (new_node->data == temp->data) {  // make sure không có bị trùng
                return false;
            }
...
protected:
    int level = 0;
    string command = "", identifier = "", type = "", value = "", statik;
    string line = "";
    bool isStatik = false;
    bool isFunction = false;
    SplayTree<Symbol> tree_symb;
    Stack<Symbol> myStack;
    Stack<Symbol> myStack2;
...
public:
    ...
    bool isValidType(string& istype);
    bool isTypeFunction(string& isTypeFunction);
    bool isStatic(string& statik);

    int isValueNumberNString(string& value); // 1 is number, 2 is string, -1 maybe function or other Iden?
    ...
    bool assignNumString(int& total_nComp, int& total_nSplay);
    bool assignIden(int& total_nComp, int& total_nSplay);
    bool assignFunc(int& total_nComp, int& total_nSplay);
    bool isMatch(string& typeA, string& str, int& total_nComp, int& total_nSplay);
    bool isValidAssignFunctionValue(Queue<string>& mykuteValue, string& str);
    bool isValidAssignFunctionIden(Queue<string>& mykuteIden, string& str);
